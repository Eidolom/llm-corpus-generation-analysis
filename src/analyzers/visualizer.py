...
# --- CONFIGURATION ---
INPUT_FILENAME = "outputs/thesis_textbook_data_filtered.csv"
...
cross_tab = pd.crosstab(df[register_col], df[usage_col])

# Keep preferred order when available, then append any other register labels.
ordered_index = [r for r in register_order if r in cross_tab.index]
ordered_index.extend([r for r in cross_tab.index if r not in ordered_index])
cross_tab = cross_tab.reindex(ordered_index, fill_value=0)
cross_tab_prop = cross_tab.div(cross_tab.sum(1), axis=0).fillna(0)
...
if cross_tab.shape[0] < 2 or cross_tab.shape[1] < 2:
    print(">> RESULT: Chi-square skipped (need at least 2 register groups and 2 usage categories).")
else:
...
pivot = (
    df.groupby([lemma_col, register_col])[usage_col]
...
if 'IDIOMATIC' in pivot.columns:
    heatmap_data = pivot['IDIOMATIC'].unstack()

    ordered_heatmap_cols = [r for r in register_order if r in heatmap_data.columns]
    ordered_heatmap_cols.extend([c for c in heatmap_data.columns if c not in ordered_heatmap_cols])
    heatmap_data = heatmap_data.reindex(columns=ordered_heatmap_cols, fill_value=0).fillna(0)
